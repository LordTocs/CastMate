...
export default {
	...
	actions: {
		request: {
			...
			async handler(data, context) {
				const [addr, body] = await Promise.all([
					await template(data.address, context),
	...
	triggers: {
		endpoint: {
			name: "HTTP Endpoint",
			description:
				"Respond to incoming HTTP Requests at /plugins/endpoints/...",
			config: {
				type: Object,
				properties: {
					method: {
