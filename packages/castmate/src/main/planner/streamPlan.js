...
export class StreamPlanManager {
	...
	async nextSegment() {
		...
		const currentSegmentIdx = this.currentPlan.config.segments.findIndex(
			(s) => s.id == this.currentSegment
		)
		if (currentSegmentIdx == -1) return false //We could get here if someone deletes the active segment in the editor!
		...
		const nextSegmentId =
			this.currentPlan.config.segments[currentSegmentIdx + 1].id
