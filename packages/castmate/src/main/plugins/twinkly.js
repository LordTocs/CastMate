	...
	methods: {
		...
		async doLogin() {
			...
			try {
				...
				const resp = await axios.post(`${this.getBaseUrl()}login`, {
					challenge: token,
				})
				...
				this.credsExpiry =
					new Date().getTime() +
					(this.creds.authentication_token_expires_in * 1000 - 5000)
		...
		isLoggedIn() {
			...
			if (!this.creds.authentication_token) return false
		...
		async setMode(mode, effectID) {
			...
			if (effectID) {
				data.effect_id =
					...
					("000000000000" + effectID.toString(16)).slice(-12)
			}
		...
		async resolveHSBK(hsbk, context) {
			...
			if (
				"bri" in hsbk &&
				(hsbk.mode == "color" || hsbk.mode == "template")
			) {
	...
	actions: {
		twinklyMovie: {
			...
			async handler(twinklyData, context) {
				this.logger.info(
					"Changing twinkly movie to " + twinklyData.movie
				)
		...
		twinklyColor: {
			...
			async handler(twinklyData, context) {
				this.logger.info(
					"Changing twinkly color to " + twinklyData.color
				)
				await this.setColor(
					await this.resolveHSBK(twinklyData.hsbk, context)
				)
