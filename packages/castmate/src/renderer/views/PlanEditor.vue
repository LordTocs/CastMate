...
<editor-shell @save="save" :name="plan?.name" :dirty="dirty">
    <flex-scroller v-if="plan" style="flex: 1">
        <div class="mx-3 my-3">
            ...
            <automation-input v-model="plan.startAutomation" label="Start Automation" />
            <automation-input v-model="plan.endAutomation" label="End Automation"/>

            <v-btn @click="addSegment" color="primary"> Add Segment </v-btn>
            <draggable v-model="plan.segments" item-key="id" handle=".segment-handle" :group="{ name: 'segment'}">
                <template #item="{element, index}">
                    <v-card class="my-4">
                        <div class="d-flex flex-row">
                            <v-sheet class="d-flex flex-column justify-center segment-handle" style="cursor: grab;">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <v-card-actions>
                                    <v-spacer />
                                    <v-btn icon="mdi-delete" size="small" color="error" @click="plan.segments.splice(index, 1)"> </v-btn>
                                </v-card-actions>
                            </div>
                        </div>
                    </v-card>
            ...
            <v-btn @click="addSegment" color="primary" v-if="plan.segments?.length"> Add Segment </v-btn>
        </div>
    </flex-scroller>
</editor-shell>
</template>
