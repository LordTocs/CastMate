...
import { ResourceImplementation, ResourceType } from "./resource"

export const ResourceRegistry = Service(
	class {
		private resourceMap = new Map<string, ResourceType>()

		getResourceType(plugin: string, id: string) {
			return this.resourceMap.get(`${plugin}_${id}`)
		}

		registerResource(resource: ResourceType) {
			this.resourceMap.set(`${resource.spec.plugin}_${resource.spec.id}`, resource)
		}
	}
)
