...
export class AutomationManager {
	...
	createIOFuncs() {
		...
		ipcFunc("io", "saveAutomation", async (name, config) => {
			try {
				await fs.promises.writeFile(
					path.join(userFolder, "automations/", `${name}.yaml`),
		...
		ipcFunc("io", "deleteAutomation", async (name) => {
			if (!(name in this.automations)) return false
		...
		ipcFunc("io", "cloneAutomation", async (name, newName) => {
			...
			try {
				await fs.promises.writeFile(
					path.join(userFolder, "automations/", `${newName}.yaml`),
					YAML.stringify(config),
					"utf-8"
				)
			} catch (err) {
	...
	async load() {
		...
		logger.info(
			`Loaded ${automationFiles.length}:${files.length} Automations`
		)

		const automations = await Promise.all(
			automationFiles.map(async (f) => {
				try {
					const str = await fs.promises.readFile(
						path.join(userFolder, "automations/", f),
						"utf-8"
					)
	...
	get(automationName) {
		return this.automations[automationName]
	}
}
