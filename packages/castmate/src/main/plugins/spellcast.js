...
import { CastMatePubSub } from "../utils/pubsub"
...
export default {
	...
	ipcMethods: {
		async checkExtensionStatus() {
			...
			if (!isPrevActive && this.state.extensionActive) {
				this.enableTriggers()
			} else if (isPrevActive && !this.state.extensionActive) {
	...
	methods: {
		enableTriggers() {
			...
			CastMatePubSub.getInstance().on(
				"spellcast.spellHook",
				this.onTrigger
			)

			this.onConnection = () => {
				CastMatePubSub.getInstance().send(
					"spellcast",
					"setActiveSpells",
					{
						spells: Array.from(this.activeButtons),
					}
				)
			}

			CastMatePubSub.getInstance().on("connection", this.onConnection)
			CastMatePubSub.getInstance().on(
				"spellcast.reinit",
				this.onConnection
			)
		...
		disableTriggers() {
			CastMatePubSub.getInstance().removeListener(
				"spellcast.spellHook",
				this.onTrigger
			)
			this.onTrigger = null
		...
		async checkExtension() {
			...
			const extensionStatus =
				await this.twitch.publicMethods.getExtensionInfo(
					SPELLCAST_EXTENSION_ID
				)
	...
	async onProfilesChanged(activeProfiles, inactiveProfiles) {
		...
		CastMatePubSub.getInstance().send("spellcast", "setActiveSpells", {
			spells: Array.from(this.activeButtons),
		})
