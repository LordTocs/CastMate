...
export class OverlayManager {
	...
	async callOverlayFunc(overlayId, widgetId, funcName, ...args) {
		await Promise.all(
			this.openSockets
				.filter((s) => s.overlayId == overlayId)
				.map(async (s) =>
					s.socket
						.call("widgetFunc", widgetId, funcName, ...args)
						.catch((err) => null)
				)
		)
	}
	...
	async broadcastOverlayFunc(funcName, ...args) {
		await Promise.all(
			this.openSockets.map(async (s) => {
				try {
					await s.socket.call("widgetBroadcast", funcName, ...args)
				} catch (err) {}
			})
		)
	}
	...
	async init() {
		...
		if (app.isPackaged) {
			...
			overlayRoutes.get(`/:id`, (req, res, next) => {
				...
				devProxy.web(req, res, {
					ignorePath: true,
					target: "http://localhost:5174/overlays/overlay.html",
				})
			})
		...
		webServices.on("ws-connection", async (socket, params) => {
			if (params.get("overlay")) {
				...
				socket.on("close", () => {
					const idx = this.openSockets.findIndex(
						(s) => s.id == newSocket.id
					)
					if (idx == -1) return
				...
				newSocket.socket.handle(
					"acquireState",
					(pluginName, stateName) => {
						if (
							newSocket.stateWatchers.find(
								(w) =>
									w.pluginName == pluginName &&
									w.stateName == stateName
							)
						)
							return stateManager.rootState[pluginName]?.[
								stateName
							]

						const watcher = onStateChange(
							stateManager.rootState[pluginName],
							stateName,
							() => {
								newSocket.socket.call(
									"stateUpdate",
									pluginName,
									stateName,
									stateManager.rootState[pluginName][
				...
				const overlay = this.overlayResources.getById(
					newSocket.overlayId
				)
				newSocket.socket.call(
					"setConfig",
					await overlay?.getTemplatedConfig()
				)
			}
		})
	}
	...
	async broadcastConfigChange(overlayId, newConfig) {
		...
		await Promise.all(
			this.openSockets
				.filter((s) => s.overlayId == overlayId)
				.map(async (s) =>
					s.socket.call("setConfig", newConfig).catch((err) => null)
				)
		)
	}
}
