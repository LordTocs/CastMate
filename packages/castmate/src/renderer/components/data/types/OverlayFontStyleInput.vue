	...
	<v-input v-model="modelObj" :density="props.density" v-bind="$attrs">
		...
		>
			<div class="v-field__input">
				<span :style="previewFontStyle" v-if="modelObj">{{
					previewText
				}}</span>
			</div>
		</v-field>
...
const fontStyle = computed(() => {
	...
	if (shadow.value) {
		result.textShadow = `${shadow.value.offsetX || 0}px ${
			shadow.value.offsetY || 0
		}px ${shadow.value.blur || 0}px ${shadow.value.color || "#000000"}`
	}
...
const previewFontStyle = computed(() => {
	...
	if (shadow.value) {
		result.textShadow = `${(shadow.value.offsetX || 0) * ratio}px ${
			(shadow.value.offsetY || 0) * ratio
		}px ${(shadow.value.blur || 0) * ratio}px ${
			shadow.value.color || "#000000"
		}`
	}
	return result
})
