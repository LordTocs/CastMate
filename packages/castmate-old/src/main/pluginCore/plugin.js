...
export class Plugin {
	constructor(config) {
		...
		for (let settingsKey in this.settings) {
			makeIPCEnumFunctions(
				this.pluginObj,
				this.name + "_settings_" + settingsKey,
		...
		for (let secretsKey in this.secrets) {
			makeIPCEnumFunctions(
				this.pluginObj,
				this.name + "_settings_" + secretsKey,
	...
	setupState(config) {
		...
		for (let stateKey in config.state) {
			...
			makeIPCEnumFunctions(
				this.pluginObj,
				this.name + "_state_" + stateKey,
				this.stateSchemas[stateKey]
			)
			this.pluginObj.state[stateKey] = constructDefaultSchema(
				config.state[stateKey]
			)
		}
	...
	setupTriggers(config) {
		...
		for (let triggerName in config.triggers) {
			...
			if (triggerSpec.config) {
				...
				triggerObj.internalHandler = triggerSpec.handler.bind(
					this.pluginObj
				)
				makeIPCEnumFunctions(
					this.pluginObj,
					this.name + "_triggers_config_" + triggerName,
					triggerSpec.config
				)
			}
			...
			this.pluginObj.triggers[triggerName] = triggerFunc.bind(
				this.pluginObj
			)
		}
	}
	...
	getUIDescription() {
		...
		for (let stateName in this.stateSchemas) {
			cleanStateSchemas[stateName] = cleanSchemaForIPC(
				this.name + "_state_" + stateName,
				this.stateSchemas[stateName]
			)
		}
		...
		for (let settingsKey in this.settings) {
			settings[settingsKey] = cleanSchemaForIPC(
				this.name + "_settings_" + settingsKey,
				this.settings[settingsKey]
			)
		}
		...
		//convert all types to strings
		for (let secretsKey in this.secrets) {
			secrets[secretsKey] = cleanSchemaForIPC(
				this.name + "_secrets_" + secretsKey,
				this.secrets[secretsKey]
			)
		}
		...
		for (let triggerName in this.triggers) {
			...
			if (triggers[triggerName].config) {
				triggers[triggerName].config = cleanSchemaForIPC(
					this.name + "_triggers_config_" + triggerName,
					triggers[triggerName].config
				)
			}

			const contextSchema = cleanSchemaForIPC(
				this.name + "_triggers_context_" + triggerName,
				{
					type: Object,
