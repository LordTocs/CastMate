<template>
    <v-container fluid>
        <link-table :items="planItems" name="Stream Plans" @nav="onNav" @create="createDlg.open()">
    ...
    <named-item-modal ref="createDlg" header="Create New Stream Plan" label="Name" @created="create($event)" />
</template>
...
const planItems = computed(() => streamplans.value.map(p => ({ id: p.id, name: p.config.name })))
...
async function tryDuplicate(item) {
    if (
        ...
        await streamPlanResource.setConfig(newPlan.id, { ...newPlan.config, name: newName})
        router.push(`/streamplans/${newPlan.id}`)
    }
}
