...
export class SettingsManager {
    ...
    async load() {
        const settingsText = await fs.promises.readFile(settingsFilePath, 'utf-8');
    ...
    async updateSettings(pluginName, settings) {
        ...
        await fs.promises.writeFile(settingsFilePath, YAML.stringify(this.settings), 'utf-8');
        ...
        if (plugin) {
            ...
            if (settings.port != oldSettings.port) { //Hardcode the port config
    ...
    async updateSecrets(pluginName, secrets) {
        ...
        await fs.promises.writeFile(secretsFilePath, YAML.stringify(this.secrets), 'utf-8');
