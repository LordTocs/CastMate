...
function migrateProfile1_0to2_0(profile, changeTracker) {
	if (profile.version != "1.0" && profile.version != undefined) return profile
	...
	for (let pluginKey in profile.triggers) {
		...
		for (let triggerKey in profile.triggers[pluginKey]) {
			...
			if (triggerKey != "follow" && triggerKey != "firstTimeChat") {
				for (let commandKey in profile.triggers[pluginKey][
					triggerKey
				]) {
					const command =
						profile.triggers[pluginKey][triggerKey][commandKey]
					...
					if (triggerKey == "chat") {
						...
						const nextCommandKey = getNextCommandKey(
							profile.triggers[pluginKey][triggerKey],
							commandKey
						)
						newCommand.config[rangeKey] = {
							min: Number(commandKey),
							max: nextCommandKey ? Number(nextCommandKey) : null,
						}
					} else if (triggerKey == "timers") {
						const [intervalStr, offsetStr] = commandKey.split("+")

						newCommand.config.delay = formatTimeString(offsetStr)
						newCommand.config.interval =
							formatTimeString(intervalStr)
					}

					newProfile.triggers[pluginKey][newTriggerKey].push(
						newCommand
					)
				}
			} else {
