...
export function handleIpcMessage(category: string, event: string, handler: (event: IpcRendererEvent, ...args: any[]) => any) {
	ipcRenderer.on(`${category}_${event}`, handler)
}

export function useIpcMessage(category: string, event: string, handler: (event: IpcRendererEvent, ...args: any[]) => void) {
