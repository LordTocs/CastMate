				...
				<v-row>
					<v-col>
						<v-card v-if="profile">
							...
							<v-card-subtitle>
								These conditions dictate whether this profile is
								active.
							</v-card-subtitle>
							<v-card-text>
								<boolean-group
							...
							<v-card-subtitle>
								These automations get run when a profile becomes
	...
	computed: {
		...
		triggerPlugins() {
			return this.pluginList.filter(
				(p) => Object.keys(p.triggers).length > 0
			)
		},
	},
	data() {
