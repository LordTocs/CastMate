	...
	<v-card>
		...
		<v-card-text v-if="triggerPlugins">
			...
			>
				<tbody>
					<template
						v-for="pluginKey in triggerPlugins"
						:key="pluginKey"
					>
						<tr
							:style="{
								backgroundColor: plugins[pluginKey].color,
							}"
						>
							<td colspan="5" class="text-h4">
								<v-icon
									:icon="plugins[pluginKey].icon"
									x-large
								/>
						...
						<template
							v-for="triggerKey in Object.keys(
								visibleTriggers[pluginKey]
							)"
						...
						>
							...
							>
								<td colspan="5" class="text-center">
									{{
										plugins[pluginKey].triggers[triggerKey]
											.name
									}}
								</td>
							</tr>
							<trigger-list-row
								...
								v-for="mapping in visibleTriggers[pluginKey][
									triggerKey
								]"
								:mapping="mapping"
								:key="mapping.id"
...
export default {
		...
		visibleTriggers() {
			...
			for (let pluginKey in this.modelValue) {
				...
				for (let triggerKey in this.modelValue[pluginKey]) {
					for (let triggerMapping of this.modelValue[pluginKey][
						triggerKey
					]) {
						if (!this.plugins[pluginKey].triggers[triggerKey]) {
							console.log(
								"UNKNOWN PLUGIN TRIGGER",
								pluginKey,
								triggerKey
							)
							continue
						...
						if (
							filterSchema(
								...
								this.plugins[pluginKey].triggers[triggerKey]
									.config,
								this.search,
								this.plugins[pluginKey].triggers[triggerKey]
									.name
	...
	methods: {
		getMappingIndex(pluginKey, triggerKey, id) {
			return this.modelValue[pluginKey][triggerKey].findIndex(
				(m) => m.id == id
			)
		},
		addMappingInternal(newValue, triggerType, mapping) {
			...
			newValue[triggerType.pluginKey][triggerType.triggerKey].push(
				mapping
			)
		},
