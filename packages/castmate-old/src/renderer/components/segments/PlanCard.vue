	...
	<v-card>
		<v-card-title class="d-flex flex-row align-center">
			...
			<template v-if="plans.length > 0">
				...
				<v-btn
					...
					class="ml-4"
					@click="start"
					v-if="!selectedPlan || currentPlan?.id != selectedPlan?.id"
				...
				<v-btn
					...
					class="ml-4"
					@click="stop"
					v-else-if="currentPlan?.id == selectedPlan?.id"
				/>
			</template>
		</v-card-title>
		...
		>
			<p class="text-h6 mb-3">
				Stream Plans are an easy way to manage titles, tags, and
				automations throughout a stream.
			</p>
			<v-btn link to="/streamplans" color="primary">
				Create a Stream Plan</v-btn
			>
		</v-card-text>
		<v-card-actions>
			<v-btn
...
const currentPlan = computed(() =>
	plans.value.find((p) => p.id == planStore.planId)
)
const currentSegment = computed(() =>
	currentPlan.value?.segments?.find((s) => s.id == planStore.segmentId)
)
...
async function start() {
	if (selectedPlan.value) await startPlan(selectedPlan.value.id)
}
