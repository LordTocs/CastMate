			...
			<flex-scroller style="flex: 1.5">
				<div v-if="widgetPropSchema">
					...
					<data-input
						...
						v-model="selectedWidgetProps"
						:colorRefs="widgetColorRefs"
					/>
					<v-divider></v-divider>
					<overlay-transform-input
						density="compact"
						v-model:size="selectedWidgetSize"
						v-model:position="selectedWidgetPosition"
					/>
				</div>
			</flex-scroller>
			<v-divider />
...
const selectedWidgetIndex = computed(() => {
	...
	if (!overlay.value) return -1

	return overlay.value?.widgets?.findIndex(
		(w) => w.id == selectedWidgetId.value
	)
})
