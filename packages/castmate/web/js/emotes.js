...
class EmoteService {
	...
	async init7tv() {
		...
		const resp = await axios.get(
			`https://api.7tv.app/v2/users/${this.channelName}/emotes`
		)
		...
		this.sevenEvents = new EventSource(
			`https://events.7tv.app/v1/channel-emotes?channel=${this.channelName}`
		)
		...
		this.sevenEvents.addEventListener(
			...
			(e) => {
				...
				if (data.action == "ADD") {
					this.sevenTVLookup[
						data.name
					] = `https://cdn.7tv.app/emote/${data.emote_id}/3x`
