...
<template>
    <v-card>
        <v-card-text v-if="discordhooks.length > 0">
            <v-list>
                <v-list-item v-for="hook in discordhooks" :key="hook.id" :title="hook.config.name">
                    <template #append>
                        <v-btn icon="mdi-delete" size="x-small" variant='flat' class="mx-1" @click="deleteHook(hook.id)" />
                        <v-btn icon="mdi-pencil" size="x-small" variant='flat' class="mx-1" @click="editHook(hook.id)" />
                    </template>
                </v-list-item>
            </v-list>
        </v-card-text>
        <v-card-text class="d-flex flex-row align-center justify-center" v-else>
            <p class="text-subtitle-1">Add a discord webhook to allow CastMate to send messages to that channel.</p>
        </v-card-text>
        <v-card-actions>
    ...
    <discord-hook-dialog ref="editDlg" title="Edit Discord WebHook" @ok="doEdit"/>
    <discord-hook-dialog ref="createDlg" title="Add Discord Webhook" @ok="doCreate"/>
</template>
