...
export const useDataInputStore = defineStore("data-components", () => {
	...
	const viewMap = ref<Map<string, Component>>(new Map())

	function registerInputComponent(type: string, component: Component) {
		componentMap.value.set(type, markRaw(component))
	}

	function registerViewComponent(type: string, component: Component) {
		viewMap.value.set(type, markRaw(component))
	}
...
export function useDataComponent(type: MaybeRefOrGetter<SchemaTypeNames>) {
	const inputStore = useDataInputStore()

	return computed(() => inputStore.getInputComponent(toValue(type)))
}

export function useDataViewComponent(type: MaybeRefOrGetter<SchemaTypeNames>) {
	const inputStore = useDataInputStore()
...
export function initData() {
	...
	inputStore.registerInputComponent("Resource", ResourceInputVue)
	...
	inputStore.registerViewComponent("Resource", ResourceViewVue)
...
export function provideDataContextSchema(schema: MaybeRefOrGetter<SchemaObject>) {
	const schemaRef = computed(() => toValue(schema))

	provide("data-context", schemaRef)
}

export function injectDataContextSchema(): ComputedRef<SchemaObject> {
	const dummyRef = computed<SchemaObject>(() => markRaw(S.Object({})))
