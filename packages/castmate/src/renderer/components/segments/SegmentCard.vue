<template>
    ...
    >
        ...
        <automation-preview size="x-small" class="mt-1 justify-center" v-if="segment.startAutomation" :automation="segment.startAutomation" />
        <v-spacer />
        <v-card-actions>
            <segment-quick-edit-dialog :plan-id="props.planId" :segment-id="props.segment.id" ref="editDlg"/>
            <v-btn size="small" icon="mdi-pencil" @click="editDlg.open()" />
            <v-spacer />
            <v-btn size="small" icon="mdi-play" color="green" @click="activate" v-if="planActive"/>
        </v-card-actions>
    </v-card>
</template>
