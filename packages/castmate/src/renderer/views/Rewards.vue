		...
		<v-table>
			<thead>
				<tr>
					...
					<th>Reward</th>
					<th>Cooldown</th>
					<th colspan="2">Max Redemptions</th>
					<th></th>
				</tr>
			</thead>
				...
				<tr v-for="(reward, i) in rewards" :key="reward.name">
					<td class="fitwidth">
						<v-tooltip v-if="reward.autoFulfill" location="right">
							...
							This reward won't show in the creator dashboard
							queue.
						</v-tooltip>
						<v-tooltip
							v-if="reward.userInputRequired"
							location="right"
						>
							<template #activator="{ props }">
								<v-icon
					...
					<td class="fitwidth text-center">
						<span v-if="reward.globalCooldown"
							>{{ reward.globalCooldown }}s</span
						>
					</td>
					<td class="fitwidth">
			...
			<tbody v-else>
				<td colspan="7" class="px-4 py-8 text-center">
					<span>
						Create some Channel Point Rewards here so CastMate can
						use them.
					</span>
				</td>
			</tbody>
