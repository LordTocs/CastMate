...
export class Overlay extends FileResource {
	...
	async getTemplatedConfig() {
		...
		result.widgets = await Promise.all(
			result.widgets.map(async (w) => ({
				id: w.id,
				type: w.type,
				name: w.name,
				size: w.size,
				position: w.position,
				props: await templateSchema(
					w.props,
					overlayManager.getWidgetType(w.type)?.props,
	...
	async resetWatcher() {
		...
		this.configWatch = await Watcher.watchAsync(async () => {
			...
			await OverlayManager.getInstance().broadcastConfigChange(
				this.id,
				config
			)
		}, "Overlay Updater")
