<template>
	<div class="d-flex flex-row justify-center">
		<div class="state-select mx-1">
			<state-selector v-model="state" label="State Value" />
		</div>
		...
		<div class="state-select mx-1">
			<data-input
				...
				:schema="stateSchema"
			/>
			<v-text-field v-model="compare" v-else />
		</div>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		stateSchema() {
			...
			if (plugin)
				schema = _cloneDeep(
					plugin.stateSchemas[this.modelValue.state.key]
				)
			if (!schema)
		...
		stateValue() {
			if (!this.modelValue || !this.modelValue.state) return undefined
			return this.rootState[this.modelValue.state.plugin][
				this.modelValue.state.key
			]
		},
	},
}
